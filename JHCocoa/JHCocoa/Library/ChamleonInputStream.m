...
@implementation ChamleonInputStream

+ (void)initialize
{
	if (self == [ChamleonInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(ChamleonHTTPRequest *)theRequest
{
	ChamleonInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(ChamleonHTTPRequest *)theRequest
{
	ChamleonInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask ChamleonHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([ChamleonHTTPRequest isBandwidthThrottled]) {
		toRead = [ChamleonHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	[ChamleonHTTPRequest incrementBandwidthUsedInLastSecond:toRead];
	[readLock unlock];
