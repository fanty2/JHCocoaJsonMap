...
#if TARGET_OS_IPHONE
#import "ChamleonReachability.h"
#import "ChamleonAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "ChamleonInputStream.h"
#import "ChamleonDataDecompressor.h"
#import "ChamleonDataCompressor.h"

// Automatically set on build
NSString *ChamleonHTTPRequestVersion = @"v1.8-95 2011-05-28";

static NSString *defaultUserAgent = nil;

NSString* const ChamleonNetworkRequestErrorDomain = @"ChamleonHTTPRequestErrorDomain";

static NSString *ChamleonHTTPRequestRunLoopMode = @"ChamleonHTTPRequestRunLoopMode";
...
const int ChamleonRedirectionLimit = 5;
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((ChamleonHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *ChamleonRequestCancelledError;
static NSError *ChamleonRequestTimedOutError;
static NSError *ChamleonAuthenticationError;
static NSError *ChamleonUnableToCreateRequestError;
static NSError *ChamleonTooMuchRedirectionError;
...
unsigned long const ChamleonWWANBandwidthThrottleAmount = 14800;
...
// When using ChamleonAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <ChamleonCacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [ChamleonHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
+ (void)performInvocation:(NSInvocation *)invocation onTarget:(id *)target releChamleonngObject:(id)objectToRelease;
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(ChamleonHTTPRequest *)theRequest;
...
#if TARGET_OS_IPHONE
+ (void)registerForNetworkChamleonReachabilityNotifications;
+ (void)unsubscribeFromNetworkChamleonReachabilityNotifications;
// Called when the status of the network changes
+ (void)ChamleonReachabilityChanged:(NSNotification *)note;
#endif

#if NS_BLOCKS_AVAILABLE
- (void)performBlockOnMainThread:(ChamleonBChamleoncBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(ChamleonBChamleoncBlock)block;
#endif
...
@property (assign) ChamleonAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) ChamleonHTTPRequest *PACFileRequest;
...
@implementation ChamleonHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [ChamleonHTTPRequest class]) {
		...
		ChamleonRequestTimedOutError = [[NSError alloc] initWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];
		ChamleonAuthenticationError = [[NSError alloc] initWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		ChamleonRequestCancelledError = [[NSError alloc] initWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		ChamleonUnableToCreateRequestError = [[NSError alloc] initWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		ChamleonTooMuchRedirectionError = [[NSError alloc] initWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonTooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[ChamleonHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <ChamleonCacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:ChamleonUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <ChamleonCacheDelegate>)cache andCachePolicy:(ChamleonCachePolicy)policy
{
	ChamleonHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:ChamleonNoAuthenticationNeededYet];
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![ChamleonDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [ChamleonDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	[self failWithError:ChamleonRequestCancelledError];
...
- (NSData *)responseData
{
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [ChamleonDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	[self setRunLoopMode:ChamleonHTTPRequestRunLoopMode];
...
- (void)main
{
	@try {
		...
		if ([ChamleonHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		// A HEAD request generated by an ChamleonNetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;
		...
		if (![self url]) {
			[self failWithError:ChamleonUnableToCreateRequestError];
			return;
		}
		...
		//If this is a HEAD request generated by an ChamleonNetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (ChamleonS3Request does this)
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (ChamleonAskServerIfModifiedWhenStaleCachePolicy|ChamleonAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)applyAuthorizationHeader
{
	...
	if (![[self requestHeaders] objectForKey:@"Authorization"]) {
		...
		// If we have bChamleonc authentication explicitly set and a username and password set on the request, add a bChamleonc auth header
		if ([self username] && [self password] && [[self authenticationScheme] isEqualToString:(NSString *)kCFHTTPAuthenticationSchemeBasic]) {
			[self addBChamleoncAuthenticationHeaderWithUsername:[self username] andPassword:[self password]];
            
#if DEBUG_HTTP_AUTHENTICATION
			NSLog(@"[AUTH] Request %@ has a username and password set, and was manually configured to use BChamleonC. Will send credentials without waiting for an authentication challenge",self);
#endif
			...
			if ([self useSessionPersistence]) {
				...
				if (credentials) {
					...
					if ([credentials objectForKey:@"Authentication"]) {
                        ...
                        // If the Authentication key is not set, these credentials were stored after a username and password set on a previous request passed bChamleonc authentication
						...
						[self addBChamleoncAuthenticationHeaderWithUsername:[usernameAndPassword objectForKey:(NSString *)kCFHTTPAuthenticationUsername] andPassword:[usernameAndPassword objectForKey:(NSString *)kCFHTTPAuthenticationPassword]];
#if DEBUG_HTTP_AUTHENTICATION
						NSLog(@"[AUTH] Request %@ found cached BChamleonC credentials from a previous request. Will send credentials without waiting for an authentication challenge",self);
#endif
					}
				}
			}
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!userAgentString) {
			userAgentString = [ChamleonHTTPRequest defaultUserAgentString];
		}
		if (userAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[ChamleonInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[ChamleonInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[ChamleonInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[ChamleonInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[(NSInputStream *)CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream]) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[ChamleonHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("ChamleonStreamID"), [[self connectionInfo] objectForKey:@"id"]);
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[ChamleonHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[ChamleonHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}
...
- (void)performRedirect
{
	...
	if ([self redirectCount] > ChamleonRedirectionLimit) {
		// Some naughty / badly coded website is trying to force us into a redirection loop. This is not cool.
		[self failWithError:ChamleonTooMuchRedirectionError];
		[self setComplete:YES];
	} else {
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {
		...
		[self failWithError:ChamleonRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[ChamleonHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];
...
// Used by ChamleonNetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (ChamleonHTTPRequest *)HEADRequest
{
	ChamleonHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[ChamleonHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[ChamleonHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[ChamleonHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[ChamleonHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];
    
	[ChamleonHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[ChamleonHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[ChamleonHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[ChamleonHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[ChamleonHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[ChamleonHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[ChamleonHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[ChamleonHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[ChamleonHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[ChamleonHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[ChamleonHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        SEL callback = @selector(performInvocation:onTarget:releChamleonngObject:);
        NSMethodSignature *cbSignature = [ChamleonHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[ChamleonHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	NSLog(@"[STATUS] Request %@: %@",self,(theError == ChamleonRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	
	// Invalidate the current connection so subsequent requests don't attempt to reuse it
	if (theError && [theError code] != ChamleonAuthenticationErrorType && [theError code] != ChamleonTooMuchRedirectionErrorType) {
		[connectionsLock lock];
#if DEBUG_PERSISTENT_CONNECTIONS
	...
	// If we have cached data, use it and ignore the error when using ChamleonFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & ChamleonFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	ChamleonHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an ChamleonNetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:ChamleonHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:ChamleonProxyAuthenticationNeeded];
	} else {
#if DEBUG_HTTP_AUTHENTICATION
	...
	if (![self authenticationNeeded]) {
		...
		// Did we get here without an authentication challenge? (which can happen when shouldPresentCredentialsBeforeChallenge is YES and bChamleonc auth was successful)
...
#if DEBUG_HTTP_AUTHENTICATION
			NSLog(@"[AUTH] Request %@ passed BChamleonC authentication, and will save credentials in the session store for future use",self);
#endif
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[ChamleonHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		ChamleonHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
...
- (BOOL)willRedirect
{
	...
	// Note that ChamleonHTTPRequest does not currently support 305 Use Proxy
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[ChamleonHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[ChamleonHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	ChamleonHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an ChamleonNetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [ChamleonHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [ChamleonHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
#if DEBUG_HTTP_AUTHENTICATION
...
{
#if DEBUG_HTTP_AUTHENTICATION
    NSLog(@"[AUTH] Request %@ received credentials from its delegate or an ChamleonAuthenticationDialog, will retry",self);
#endif
...
{
#if DEBUG_HTTP_AUTHENTICATION
    NSLog(@"[AUTH] Request %@ had authentication cancelled by its delegate or an ChamleonAuthenticationDialog",self);
#endif
...
- (void)failAuthentication
{
	[self failWithError:ChamleonAuthenticationError];
}
...
- (BOOL)showProxyAuthenticationDialog
{
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		[ChamleonAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		if ((([self proxyAuthenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self proxyAuthenticationRetryCount] < 2) && [self applyProxyCredentials:proxyCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == ChamleonProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
...
#if DEBUG_HTTP_AUTHENTICATION
				NSLog(@"[AUTH] Request %@ will ask ChamleonAuthenticationDialog for credentials",self);
#endif
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)addBChamleoncAuthenticationHeaderWithUsername:(NSString *)theUsername andPassword:(NSString *)thePassword
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"BChamleonc %@",[ChamleonHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];
    ...
    if (bytesRead < 0) {
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[ChamleonDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[ChamleonHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
- (void)handleStreamComplete
{
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[ChamleonDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
	...
	if (![self needsRedirect] && ![self authenticationNeeded] && ![self didUseCachedResponse]) {
        ...
        // If request has asked delegate or ChamleonAuthenticationDialog for credentials
...
- (void)markAsFinished
{
    ...
    BOOL wChamleonnProgress = inProgress;
    BOOL wasFinished = finished;
    
    if (!wasFinished)
    [self willChangeValueForKey:@"isFinished"];
    if (wChamleonnProgress)
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-ChamleonHTTPRequest-Response-Status-Code"] intValue]];
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (ChamleonHTTPRequest does not support file:// urls)
	...
	// Create an ChamleonHTTPRequest to fetch the PAC file
	ChamleonHTTPRequest *PACRequest = [ChamleonHTTPRequest requestWithURL:[self PACurl]];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	ChamleonHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
{
	if (!sessionCookies) {
		[ChamleonHTTPRequest setSessionCookies:[[[NSMutableArray alloc] init] autorelease]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[ChamleonHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[ChamleonHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[ChamleonHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[ChamleonHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:ChamleonCacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	...
	// If we couldn't find one, we'll give up (and ChamleonHTTPRequest will use the standard CFNetwork user agent)
	if (!appName) {
		return nil;
	}
	...
	// We won't bother to check for systems prior to 10.4, since ChamleonHTTPRequest only works on 10.5+
    OSErr err;
...
- (void)performThrottling
{
	...
	[ChamleonHTTPRequest measureBandwidthUsage];
	if ([ChamleonHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[ChamleonHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an ChamleonNetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[ChamleonHTTPRequest throttleBandwidthForWWANUsingLimit:ChamleonWWANBandwidthThrottleAmount];
	} else {
		[ChamleonHTTPRequest unsubscribeFromNetworkChamleonReachabilityNotifications];
		[ChamleonHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
+ (void)throttleBandwidthForWWANUsingLimit:(unsigned long)limit
{
	...
	[ChamleonHTTPRequest registerForNetworkChamleonReachabilityNotifications];
	[bandwidthThrottlingLock unlock];
	[ChamleonHTTPRequest ChamleonReachabilityChanged:nil];
}

#pragma mark ChamleonReachability

+ (void)registerForNetworkChamleonReachabilityNotifications
{
	[[ChamleonReachability ChamleonReachabilityForInternetConnection] startNotifier];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ChamleonReachabilityChanged:) name:kChamleonReachabilityChangedNotification object:nil];
}


+ (void)unsubscribeFromNetworkChamleonReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:kChamleonReachabilityChangedNotification object:nil];
}

+ (BOOL)isNetworkReachableViaWWAN
{
	return ([[ChamleonReachability ChamleonReachabilityForInternetConnection] currentChamleonReachabilityStatus] == ReachableViaWWAN);
}

+ (void)ChamleonReachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [ChamleonHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <ChamleonCacheDelegate>)cache
{
	[defaultCache release];
	defaultCache = [cache retain];
}

+ (id <ChamleonCacheDelegate>)defaultCache
{
	return defaultCache;
}
...
+ (NSThread *)threadForRequest:(ChamleonHTTPRequest *)request
{
	if (!networkThread) {
...
#pragma mark blocks
#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(ChamleonBChamleoncBlock)aStartedBlock
{
	[startedBlock release];
	startedBlock = [aStartedBlock copy];
}

- (void)setHeadersReceivedBlock:(ChamleonHeadersBlock)aReceivedBlock
{
	[headersReceivedBlock release];
	headersReceivedBlock = [aReceivedBlock copy];
}

- (void)setCompletionBlock:(ChamleonBChamleoncBlock)aCompletionBlock
{
	[completionBlock release];
	completionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(ChamleonBChamleoncBlock)aFailedBlock
{
	[failureBlock release];
	failureBlock = [aFailedBlock copy];
}

- (void)setBytesReceivedBlock:(ChamleonProgressBlock)aBytesReceivedBlock
{
	[bytesReceivedBlock release];
	bytesReceivedBlock = [aBytesReceivedBlock copy];
}

- (void)setBytesSentBlock:(ChamleonProgressBlock)aBytesSentBlock
{
	[bytesSentBlock release];
	bytesSentBlock = [aBytesSentBlock copy];
}

- (void)setDownloadSizeIncrementedBlock:(ChamleonSizeBlock)aDownloadSizeIncrementedBlock{
	[downloadSizeIncrementedBlock release];
	downloadSizeIncrementedBlock = [aDownloadSizeIncrementedBlock copy];
}

- (void)setUploadSizeIncrementedBlock:(ChamleonSizeBlock)anUploadSizeIncrementedBlock
{
	[uploadSizeIncrementedBlock release];
	uploadSizeIncrementedBlock = [anUploadSizeIncrementedBlock copy];
}

- (void)setDataReceivedBlock:(ChamleonDataBlock)aReceivedBlock
{
	[dataReceivedBlock release];
	dataReceivedBlock = [aReceivedBlock copy];
}

- (void)setAuthenticationNeededBlock:(ChamleonBChamleoncBlock)anAuthenticationBlock
{
	[authenticationNeededBlock release];
	authenticationNeededBlock = [anAuthenticationBlock copy];
}
- (void)setProxyAuthenticationNeededBlock:(ChamleonBChamleoncBlock)aProxyAuthenticationBlock
{
	[proxyAuthenticationNeededBlock release];
	proxyAuthenticationNeededBlock = [aProxyAuthenticationBlock copy];
}
- (void)setRequestRedirectedBlock:(ChamleonBChamleoncBlock)aRedirectBlock
{
