...
typedef enum _ChamleonPostFormat {
    ChamleonMultipartFormDataPostFormat = 0,
    ChamleonURLEncodedPostFormat = 1
	
} ChamleonPostFormat;

@interface ChamleonFormDataRequest : ChamleonHTTPRequest <NSCopying> {
	...
	ChamleonPostFormat postFormat;
	
	NSStringEncoding stringEncoding;
	
#if DEBUG_FORM_DATA_REQUEST
	// Will store a string version of the request body that will be printed to the console when ChamleonHTTPREQUEST_DEBUG is set in GCC_PREPROCESSOR_DEFINITIONS
...
@property (assign) ChamleonPostFormat postFormat;
