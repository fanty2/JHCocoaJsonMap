...
#import <stdio.h>
#import "ChamleonHTTPRequestDelegate.h"

@class ChamleonDataDecompressor;

extern NSString *ChamleonHTTPRequestVersion;
...
typedef enum _ChamleonAuthenticationState {
	ChamleonNoAuthenticationNeededYet = 0,
	ChamleonHTTPAuthenticationNeeded = 1,
	ChamleonProxyAuthenticationNeeded = 2
} ChamleonAuthenticationState;

typedef enum _ChamleonNetworkErrorType {
    ChamleonConnectionFailureErrorType = 1,
    ChamleonRequestTimedOutErrorType = 2,
    ChamleonAuthenticationErrorType = 3,
    ChamleonRequestCancelledErrorType = 4,
    ChamleonUnableToCreateRequestErrorType = 5,
    ChamleonInternalErrorWhileBuildingRequestType  = 6,
    ChamleonInternalErrorWhileApplyingCredentialsType  = 7,
	ChamleonFileManagementError = 8,
	ChamleonTooMuchRedirectionErrorType = 9,
	ChamleonUnhandledExceptionError = 10,
	ChamleonCompressionError = 11
	
} ChamleonNetworkErrorType;


// The error domain that all errors generated by ChamleonHTTPRequest use
extern NSString* const ChamleonNetworkRequestErrorDomain;
...
extern unsigned long const ChamleonWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^ChamleonBChamleoncBlock)(void);
typedef void (^ChamleonHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^ChamleonSizeBlock)(long long size);
typedef void (^ChamleonProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^ChamleonDataBlock)(NSData *data);
#endif

@interface ChamleonHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the ChamleonHTTPRequestDelegate protocol
	id <ChamleonHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but ChamleonNetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <ChamleonHTTPRequestDelegate, ChamleonProgressDelegate> queue;
	...
	// Automatically set to true in ChamleonFormDataRequests when using setFile:forKey:
	...
	// Set to true when ChamleonHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = ChamleonConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <ChamleonProgressDelegate> uploadProgressDelegate;
	...
	id <ChamleonProgressDelegate> downloadProgressDelegate;
	...
	// Authentication scheme (BChamleonc, Digest, NTLM)
	// If you are using BChamleonc authentication and want to force ChamleonHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBChamleonc
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, ChamleonHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, ChamleonHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	// The dialog will not be shown if your delegate responds to proxyAuthenticationNeededForRequest:
	// Default is YES (bChamleoncally, because most people won't want the hassle of adding support for authenticating proxies to their apps)
	...
	// Authentication scheme for the proxy (BChamleonc, Digest, NTLM)
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - ChamleonHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	ChamleonHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in ChamleonNetworkQueue.h
	...
	// Tells ChamleonHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let ChamleonHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// ChamleonHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See ChamleonAuthenticationState values above. 0 == default == No authentication needed yet
	ChamleonAuthenticationState authenticationNeeded;
	
	// When YES, ChamleonHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Default is YES
	// For requests using BChamleonc authentication, set authenticationScheme to (NSString *)kCFHTTPAuthenticationSchemeBChamleonc, and credentials can be sent on the very first request when shouldPresentCredentialsBeforeChallenge is YES
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops ChamleonHTTPRequest cleaning up a temporary post body)
	...
	// Will be ChamleonHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [ChamleonHTTPRequest setDefaultCache:cache] to configure a default cache
	id <ChamleonCacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See ChamleonCacheDelegate.h for possible values
	ChamleonCachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See ChamleonCacheDelegate.h for possible values
	ChamleonCacheStoragePolicy cacheStoragePolicy;
	...
	ChamleonDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using ChamleonHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	ChamleonHTTPRequest *PACFileRequest;
	...
	ChamleonBChamleoncBlock startedBlock;
    
	//block to execute when headers are received
	ChamleonHeadersBlock headersReceivedBlock;
    
	//block to execute when request completes successfully
	ChamleonBChamleoncBlock completionBlock;
    
	//block to execute when request fails
	ChamleonBChamleoncBlock failureBlock;
    
	//block for when bytes are received
	ChamleonProgressBlock bytesReceivedBlock;
    
	//block for when bytes are sent
	ChamleonProgressBlock bytesSentBlock;
    
	//block for when download size is incremented
	ChamleonSizeBlock downloadSizeIncrementedBlock;
    
	//block for when upload size is incremented
	ChamleonSizeBlock uploadSizeIncrementedBlock;
    
	//block for handling raw bytes received
	ChamleonDataBlock dataReceivedBlock;
    
	//block for handling authentication
	ChamleonBChamleoncBlock authenticationNeededBlock;
    
	//block for handling proxy authentication
	ChamleonBChamleoncBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    ChamleonBChamleoncBlock requestRedirectedBlock;
#endif
}
...
// Construct a bChamleonc authentication header from the username and password supplied, and add it to the request headers
...
// Used internally to record bandwidth use, and by ChamleonInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, ChamleonHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
// Used for generating Authorization header when using bChamleonc authentication when shouldPresentCredentialsBeforeChallenge is true
// And also by ChamleonS3Request
...
@property (assign) ChamleonHTTPRequest *mainRequest;
...
@property (assign, readonly) ChamleonAuthenticationState authenticationNeeded;
...
@property (assign) id <ChamleonCacheDelegate> downloadCache;
@property (assign) ChamleonCachePolicy cachePolicy;
@property (assign) ChamleonCacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) ChamleonDataDecompressor *dataDecompressor;
