...
// Private stuff
@interface ChamleonFormDataRequest ()
...
@implementation ChamleonFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:ChamleonURLEncodedPostFormat];
...
- (void)addFile:(id)data withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if ([data isKindOfClass:[NSString class]]) {
		...
		if (!fileExists || isDirectory) {
			[self failWithError:[NSError errorWithDomain:ChamleonNetworkRequestErrorDomain code:ChamleonInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",data],NSLocalizedDescriptionKey,nil]]];
		}
		...
		if (!contentType) {
			contentType = [ChamleonHTTPRequest mimeTypeForFileAtPath:data];
		}
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == ChamleonURLEncodedPostFormat) {
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:ChamleonMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	ChamleonFormDataRequest *newRequest = [super copyWithZone:zone];
